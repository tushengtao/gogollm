...
@router.get('/access', summary='获取用户菜单权限', dependencies=[DependsJwtAuth])
async def get_user_menus_access(request: Request) -> ResponseModel:
    access = await menu_service.get_user_menu_access(request=request)
    return await response_base.success(data=access)
