...
class CRUDDept(CRUDBase[Dept, CreateDeptParam, UpdateDeptParam]):
    async def get(self, db: AsyncSession, dept_id: int) -> Dept | None:
        ...
        :return:
        """
        return await self.get_(db, pk=dept_id, del_flag=False)
    ...
    async def get_by_name(self, db: AsyncSession, name: str) -> Dept | None:
        ...
        :return:
        """
        return await self.get_(db, name=name, del_flag=False)
    ...
    async def get_all(
        ...
        where_list = [self.model.del_flag == False]
        conditions = []
        if name:
    ...
    async def delete(self, db: AsyncSession, dept_id: int) -> int:
        ...
        :return:
        """
        return await self.delete_(db, dept_id, del_flag=True)
