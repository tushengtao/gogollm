...
class CRUDBase(Generic[ModelType, CreateSchemaType, UpdateSchemaType]):
    ...
    async def get_(
        ...
        if del_flag is not None:
            assert del_flag in (True, False), '查询错误, del_flag 参数只能为True, False'
    ...
    async def delete_(self, db: AsyncSession, pk: int, *, del_flag: bool | None = None) -> int:
        """
        通过主键 id 删除一条数据
        ...
        if del_flag is None:
            ...
            assert del_flag == True, '删除错误, del_flag 参数只能为 true'
